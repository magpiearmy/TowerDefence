...
public class ElementProperties {
    ...
    public CompoundElementType getCompoundType() {
        if (types.containsAll(Arrays.asList(FIRE, EARTH))) return CompoundElementType.LAVA;
        if (types.containsAll(Arrays.asList(FIRE, WATER))) return CompoundElementType.STEAM;
        if (types.containsAll(Arrays.asList(EARTH, WATER))) return CompoundElementType.MUD;
        return null;
    }
}
