...
public class Level {
  ...
  public void placeTower(int clickX, int clickY, TowerType type) {
    ...
    if (canPlaceTowerAtTile(mapX, mapY)) {
      ...
      if (money >= cost) {
        ...
        System.out.println(
          new StringBuilder("Cannot afford that tower! Cost [" + cost + "] Funds [" + money + "]")
            .toString());
      }
    }
  }
  ...
  public void draw(Graphics2D mapGfx, Graphics2D infoboxGfx) {
    ...
    infoboxGfx.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
